    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button className="btn-success" onClick={handleSave} disabled={saving || students.length === 0}>
          <Save className="w-4 h-4 mr-1" /> {saving ? 'Saving...' : 'Save Marks'}
        </button>
      </div>
        ...
        >
          ...
          {exams.map((exam) => (
            <option key={exam.id} value={exam.id}>
              {exam.name} - {exam.Class?.name} - {exam.Subject?.name} ({exam.exam_date})
            </option>
          ))}
        </select>
      </div>
      ...
      {selectedExam ? (
        <div className="card overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
            ...
            <h2 className="font-semibold">{students.length} Students Ready for Marks</h2>
          </div>
          <Table
            columns={columns}
            data={students}
