...
import { formatDate, getFullName, getInitialsFromName } from '../../utils/helpers';
...
const AttendancePage = () => {
  ...
  const loadStudentsByClass = async (classId) => {
    ...
    try {
      ...
      const attendanceRes = await attendanceAPI.getByClassAndDate(classId, date);
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-sm">{getFullName(row.first_name, row.last_name)}</p>
            <p className="text-xs text-gray-500">{row.roll_number || row.student_id}</p>
          </div>
        </div>
      ),
    },
  ...
  const summary = {
    ...
    excused: students.filter(s => attendanceRecords[s.id] === 'excused').length,
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button className="btn-success" onClick={handleSave} disabled={saving || students.length === 0}>
          <Save className="w-4 h-4 mr-1" /> {saving ? 'Saving...' : 'Save Attendance'}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="date" className="input" value={date} onChange={(e) => {
              ...
              if (selectedClass) loadStudentsByClass(selectedClass);
            }} />
          </div>
          <div className="flex items-end">
            <div className="flex gap-3 text-sm">
              <span className="flex items-center gap-1 text-green-600"><Check className="w-4 h-4" /> {summary.present}</span>
              <span className="flex items-center gap-1 text-red-600"><X className="w-4 h-4" /> {summary.absent}</span>
              <span className="flex items-center gap-1 text-yellow-600"><Clock className="w-4 h-4" /> {summary.late}</span>
            </div>
          </div>
        </div>
      </div>
